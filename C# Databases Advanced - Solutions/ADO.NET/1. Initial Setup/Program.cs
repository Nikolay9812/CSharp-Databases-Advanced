    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                var createTable = GetCreateTableQuery();
                var insertTable = GetInsertTableQuery();

                //ExecuteNonQuery(connection, createDatabase);

                //foreach (var query in createTableQuery) { ExecuteNonQuery(connection, query); }

                foreach (var query in insertTable) { ExecuteNonQuery(connection, query); }
